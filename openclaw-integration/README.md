...
把定制版 openclaw（`openclaw-custom/`，含内置 A2A Gateway）作为**离线、自包含**的 Agent Runtime 集成进 macOS App `MutualInfectionMac`，使互传 App 充当 A2A（Agent-to-Agent）节点。**不依赖用户本地 Node/npm/网络**——Node 二进制 + openclaw 代码 + 全部依赖都打包进 `.app`。
...
- **复用官方验证过的技巧**：`pnpm install --node-linker=hoisted`（扁平化依赖，见 `openclaw-custom/scripts/package-mac-app.sh:118`）；`OPENCLAW_HOME`/`OPENCLAW_STATE_DIR`/`OPENCLAW_CONFIG_PATH` 重定向运行时目录；`/.well-known/agent-card.json` 作为 ready/健康端点。
...
| 阶段 | 文档 | 状态 |
|------|------|------|
| 0 — openclaw-custom 本地基线跑通 | [PHASE0_基线本地跑通.md](./PHASE0_基线本地跑通.md) | ✅ 完成 |
