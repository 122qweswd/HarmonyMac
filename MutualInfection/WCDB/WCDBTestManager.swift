//
//  WCDBTestManager.swift
//  MutualInfection
//
//  Created by Niko on 2025/9/27.
//  用于测试MITransferRecord和MIWCDBManager的测试代码
import Foundation
import WCDBSwift
import UIKit
import Photos
...
struct WCDBTestConfig {
    ...
    private static func createImageFile(at url: URL) -> URL? {
        ...
        let colors: [UIColor] = [
            ...
            .orange, .systemPink, .cyan, .magenta, .brown
        ]
        ...
        let watermarkRect = CGRect(x: 20, y: size.height - 60, width: size.width - 40, height: 30)
        watermark.draw(with: watermarkRect, options: .usesLineFragmentOrigin, attributes: watermarkAttributes, context: nil)
        
        guard let image = UIGraphicsGetImageFromCurrentImageContext() else { return nil }
        
        // 根据文件扩展名保存为相应格式
        if let imageData = url.pathExtension.lowercased() == "png" ? image.pngData() : image.jpegData(compressionQuality: 0.8) {
            do {
    ...
    static func generateRandomRecord(transferType: MITransferType) async throws -> MITransferRecord? {
        ...
        record.deviceId = WCDBTestConfig.testDeviceIds.randomElement()!
        record.deviceName = WCDBTestConfig.testDeviceNames.randomElement()!
...
// MARK: - WCDB测试管理器
class WCDBTestManager {
    
    static let shared = WCDBTestManager()
    ...
    func cleanupTestFiles() {
        do {
            ...
            let testFiles = try fileManager.contentsOfDirectory(at: WCDBTestConfig.testFilesDirectory, includingPropertiesForKeys: nil)
    ...
    func testInsertSingleRecord() async -> Bool {
        print("=== 测试插入单个记录 ===")
        do {
            guard let record = try await WCDBTestConfig.generateRandomRecord(transferType: .send) else {
                print("❌ 生成测试记录失败")
                return false
            }
    ...
    func testBatchInsertRecords() async -> Bool {
        ...
        for i in 0..<recordCount {
            ...
            do {
                guard let record = try await WCDBTestConfig.generateRandomRecord(transferType: transferType) else {
                    print("❌ 记录 \(i+1) 生成失败")
                    continue
                }
                let recordId = try await dbManager.insertRecordAsync(record)
