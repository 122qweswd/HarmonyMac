    ...
    DFileSession *session, DFileRebuildFileList *rebuildList)
{
    ...
    for (uint32_t i = 0; i < fileNum; i++) {
        ...
        if (session->fileManager->fileCallback != NULL) {
            uint64_t fileSize = 0;
            session->fileManager->fileCallback(session->sessionId, FILE_GET_STAT, files[i], &fileSize, 0);
            rebuildList->fileSize[i] = fileSize;
        }
    }
    return NSTACKX_EOK;
}
