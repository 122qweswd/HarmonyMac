...
struct DFileSession {
    ...
    uint32_t decContinuousError;
    uint8_t localUseHeaderEnc;
    uint8_t remoteUseHeaderEnc;
    uint8_t settingUseHeaderEnc;
    CryptPara cryptPara;
    uint64_t lastCtrlMsgTs;
    uint8_t plaintBuff[NSTACKX_DEFAULT_FRAME_SIZE];
    uint8_t sendBuff[NSTACKX_DEFAULT_FRAME_SIZE];
};
...
static inline bool UseHeaderEnc(const struct DFileSession *session)
{
    return session != NULL && session->settingUseHeaderEnc == NSTACKX_TRUE && session->fileManager->keyLen > 0;
}

static inline bool CapsEncryptCtrlMsg(const struct DFileSession *session)
{
    return (session->capsCheck & NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG);
}
