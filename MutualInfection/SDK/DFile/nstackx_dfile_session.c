...
static QueueNode *CreateQueueNode(DFileSession *session, const uint8_t *frame, size_t length,
    const struct sockaddr_in *peerAddr, uint8_t socketIndex)
{
    QueueNode *queueNode = NULL;

    if (frame == NULL || length == 0 || length > session->maxFrameSize) {
        return NULL;
    }
...
void DFileSessionSendSetting(PeerInfo *peerInfo)
{
    ...
    settingFramePara.dataFrameSize = peerInfo->session->maxFrameSize;
    settingFramePara.capsCheck = NSTACKX_INTERNAL_CAPS_RECV_FEEDBACK;
#if defined(NSTACKX_DEVICE_L2)
    settingFramePara.capsCheck |= NSTACKX_INTERNAL_CAPS_DEVICE_L2;
#endif
    settingFramePara.capsCheck |= peerInfo->session->internalCaps & NSTACKX_INTERNAL_CAPS_CLEAR_POLICY;
    settingFramePara.capsCheck |= peerInfo->session->internalCaps & NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG;

    settingFramePara.headerEnc = peerInfo->session->localUseHeaderEnc;
...
static void UpdateDFileSessionHeaderEnc(DFileSession *session, uint8_t remoteUseHeaderEnc)
{
    if (session->settingUseHeaderEnc == NSTACKX_TRUE) {
        DFILE_LOGE(TAG, "UseHeaderEnc %u/%u/%u has set but remote request to update %u",
        session->localUseHeaderEnc, session->remoteUseHeaderEnc, session->settingUseHeaderEnc, remoteUseHeaderEnc);
        return;
    }

    session->remoteUseHeaderEnc = remoteUseHeaderEnc;
    if (session->localUseHeaderEnc == NSTACKX_TRUE && session->remoteUseHeaderEnc == NSTACKX_TRUE) {
        session->settingUseHeaderEnc = NSTACKX_TRUE;
    }
    DFILE_LOGI(TAG,"UseHeaderEnc %u/%u/%u has set but remote request to update %u",
        session->localUseHeaderEnc, session->remoteUseHeaderEnc, session->settingUseHeaderEnc, remoteUseHeaderEnc);
}

static void ClientHandleSettingCaps(const SettingFrame *hostSettingFrame, DFileSession *session)
{
//    if (hostSettingFrame->capability & NSTACKX_CAPS_LINK_SEQUENCE) {
//        DFILE_LOGI(TAG, "server replies support Link Sequence");
//    } else {
//        DFILE_LOGI(TAG, "server replies using normal ACK");
//        session->capability &= ~NSTACKX_CAPS_LINK_SEQUENCE;
//    }
//
//    if (hostSettingFrame->capability & NSTACKX_CAPS_NO_RTT) {
//        DFILE_LOGI(TAG, "server replies use No Rtt");
//    } else {
//        DFILE_LOGI(TAG, "server replies use Normal Rtt");
//        session->capability &= ~NSTACKX_CAPS_NO_RTT;
//    }
//
//    if (hostSettingFrame->capability & NSTACKX_CAPS_RESUMABLE_TRANS) {
//        DFILE_LOGI(TAG, "server replies use resumable");
//    } else {
//        DFILE_LOGI(TAG, "server replies no use resumable");
//        session->capability &= ~NSTACKX_CAPS_RESUMABLE_TRANS;
//    }
//
//    if (hostSettingFrame->capability & NSTACKX_CAPS_ALG_NORATE) {
//        session->capability |= NSTACKX_CAPS_ALG_NORATE; // client can enable norate alg by both server and itself
//        DFILE_LOGI(TAG, "server replies use NoRate algorithm");
//    }

    if ((hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_CLEAR_POLICY) &&
        (session->internalCaps & NSTACKX_INTERNAL_CAPS_CLEAR_POLICY)) {
        session->capsCheck |= NSTACKX_INTERNAL_CAPS_CLEAR_POLICY;
        DFILE_LOGI(TAG, "server support clear policy");
    }

    if ((hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG) &&
        (session->internalCaps & NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG)) {
        session->capsCheck |= NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG;
        DFILE_LOGI(TAG, "server support encrypt ctrl msg");
    }

//    if (hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_CRC_CHECK) {
//        DFILE_LOGI(TAG, "server support crc check");
//    } else {
//        DFILE_LOGI(TAG, "server do not support crc check");
//        session->capsCheck &= ~NSTACKX_INTERNAL_CAPS_CRC_CHECK;
//    }
//
//    if (hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_DEVICE_L2) {
//        DFILE_LOGI(TAG, "server device is L2");
//        session->capsCheck |= NSTACKX_INTERNAL_CAPS_DEVICE_L2;
//    }
//
//    if ((hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_EMPTY_DIR) &&
//        (session->internalCaps & NSTACKX_INTERNAL_CAPS_EMPTY_DIR)) {
//        session->capsCheck |= NSTACKX_INTERNAL_CAPS_EMPTY_DIR;
//        DFILE_LOGI(TAG, "server support empty dir");
//    }

    DFILE_LOGI(TAG, "handle server setting, current caps encryptCtrlMsg:%d", CapsEncryptCtrlMsg(session));
}
...
static void DFileSessionHandleClientSetting(DFileSession *session, DFileFrame *dFileFrame, struct sockaddr_in *peerAddr)
{
    ...
    UpdateDFileSessionHeaderEnc(session, hostSettingFrame.headerEnc);
    ...
    ClientHandleSettingCaps(&hostSettingFrame, session);
    ...
    if (GetDFileConfig(&dFileConfig, peerInfo->mtuInuse, hostSettingFrame.connType, hostSettingFrame.dataFrameSize) == NSTACKX_EOK) {
...
static int32_t SetSessionCryptPara(DFileSession *session, const uint8_t key[], uint32_t keyLen)
{
    uint32_t keyLenNew = keyLen;
    if (CapsChaCha(session)) {
        session->cryptPara.cipherType = CIPHER_CHACHA;
    } else {
        session->cryptPara.cipherType = CIPHER_AES_GCM;
        keyLenNew = AES_128_KEY_LENGTH;
    }

    if (memcpy_s(session->cryptPara.key, sizeof(session->cryptPara.key), key, keyLenNew) != EOK) {
        DFILE_LOGE(TAG, "memcpy key failed");
        return NSTACKX_EFAILED;
    }
    session->cryptPara.keylen = keyLenNew;

    uint32_t aadLen = sizeof(session->cryptPara.aad);
    if (memset_s(session->cryptPara.aad, aadLen, GCM_AAD_CHAR, aadLen) != EOK) {
        LOGE(TAG, "memset aad failed");
        return NSTACKX_EFAILED;
    }
    session->cryptPara.aadLen = aadLen;
    session->cryptPara.ctx = CreateCryptCtx();
    if (session->cryptPara.ctx == NULL) {
        DFILE_LOGE(TAG, "failed to create crypt ctx");
        return NSTACKX_EFAILED;
    }
    LOGI(TAG, "set encrypt/decrypt type is %d", session->cryptPara.cipherType);
    return NSTACKX_EOK;
}

static void ServerHandleSettingCaps(DFileSession *session, SettingFrame *hostSettingFrame)
{
//    if (hostSettingFrame->capability & NSTACKX_CAPS_LINK_SEQUENCE) {
//        DFILE_LOGI(TAG, "client wants to enable Link Sequence");
//    } else {
...
//    if (hostSettingFrame->capability & NSTACKX_CAPS_NO_RTT) {
//        DFILE_LOGI(TAG, "client wants to use No Rtt");
//    } else {
//        DFILE_LOGI(TAG, "client wants to use Normal Rtt");
//        session->capability &= ~NSTACKX_CAPS_NO_RTT;
//    }
//
//    if (hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_RECV_FEEDBACK) {
//        DFILE_LOGI(TAG, "client support recv feedback");
//        session->capsCheck |= NSTACKX_INTERNAL_CAPS_RECV_FEEDBACK;
//    } else {
//        DFILE_LOGI(TAG, "client do not support recv feedback");
...
//
//    if (hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_CRC_CHECK) {
//        DFILE_LOGI(TAG, "client support crc check");
//    } else {
//        DFILE_LOGI(TAG, "client do not support crc check");
//        session->capsCheck &= ~NSTACKX_INTERNAL_CAPS_CRC_CHECK;
//    }
//
//    if (hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_DEVICE_L2) {
//        DFILE_LOGI(TAG, "client device is L2");
//        session->capsCheck |= NSTACKX_INTERNAL_CAPS_DEVICE_L2;
//    }
//
//    if (hostSettingFrame->capability & NSTACKX_CAPS_RESUMABLE_TRANS) {
//        DFILE_LOGI(TAG, "client wants to enable resumable");
//    } else {
//        DFILE_LOGI(TAG, "client do not enable resumable");
//        session->capability &= ~NSTACKX_CAPS_RESUMABLE_TRANS;
//    }
//
//    if ((hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_EMPTY_DIR) &&
//        (session->internalCaps & NSTACKX_INTERNAL_CAPS_EMPTY_DIR)) {
//        session->capsCheck |= NSTACKX_INTERNAL_CAPS_EMPTY_DIR;
//        DFILE_LOGI(TAG, "client support empty dir");
//    }

    if ((hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_CLEAR_POLICY) &&
        (session->internalCaps & NSTACKX_INTERNAL_CAPS_CLEAR_POLICY)) {
        session->capsCheck |= NSTACKX_INTERNAL_CAPS_CLEAR_POLICY;
        DFILE_LOGI(TAG, "client support clear policy");
    }

    if ((hostSettingFrame->capsCheck & NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG) &&
        (session->internalCaps & NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG)) {
        session->capsCheck |= NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG;
        DFILE_LOGI(TAG, "client support encrypt ctrl msg");
    }
}
...
L_END:
    ...
    ServerHandleSettingCaps(session, &hostSettingFrame);
    DFileChooseCipherType(&hostSettingFrame, session);
}
    ...
    struct sockaddr_in *peerAddr, uint8_t socketIndex)
{
    ...
    if (CapsEncryptCtrlMsg(session) &&
        SetSessionCryptPara(session, session->fileManager->key, session->fileManager->keyLen) != NSTACKX_EOK) {
        DFILE_LOGE(TAG, "create session cryptpara failed");
        DFileMsg data;
        (void)memset_s(&data, sizeof(data), 0, sizeof(data));
        data.errorCode = NSTACKX_EFAILED;
...
int32_t DFileWriteHandle(const uint8_t *frame, size_t len, void *context)
{
    ...
    queueNode = CreateQueueNode(session, frame, len, &peerAddr, peerInfo->socketIndex);
    if (queueNode == NULL) {
...
static bool IsCtrlMsg(DFileFrame *frame)
{
    return (frame->header.type == NSTACKX_DFILE_FILE_HEADER_FRAME ||
        frame->header.type == NSTACKX_DFILE_FILE_HEADER_FRAME_V2 ||
        frame->header.type == NSTACKX_DFILE_FILE_HEADER_CONFIRM_FRAME ||
        frame->header.type == NSTACKX_DFILE_FILE_DATA_ACK_FRAME ||
        frame->header.type == NSTACKX_DFILE_FILE_TRANSFER_DONE_FRAME ||
        frame->header.type == NSTACKX_DFILE_FILE_TRANSFER_DONE_ACK_FRAME ||
        frame->header.type == NSTACKX_DFILE_PEER_DOWN_FRAME ||
        frame->header.type == NSTACKX_DFILE_FILE_BACK_PRESSURE_FRAME ||
        frame->header.type == NSTACKX_DFILE_FILEID_SUCC_FRAME ||
        frame->header.type == NSTACKX_DFILE_SESSION_OPT_FRAEM);
}

static bool IsCtrlMsgVaild(DFileSession *session, uint8_t *decryptBuff, DFileFrame *frame)
{
    DFileFrameHeader *decryptHdr = (DFileFrameHeader *)(decryptBuff + DFILE_CTRL_FRAME_TS_LEN);
    if (decryptHdr->type != frame->header.type || decryptHdr->flag != frame->header.flag ||
        decryptHdr->sessionId != frame->header.sessionId || decryptHdr->transId != frame->header.transId) {
        DFILE_LOGE(TAG, "header is not same");
        return false;
    }
    if (ntohs(decryptHdr->length) > session->maxFrameSize - sizeof(DFileFrameHeader)) {
        DFILE_LOGE(TAG, "header length %u is too big", ntohs(decryptHdr->length));
        return false;
    }
    uint64_t timeStamp = be64toh(*(uint64_t *)decryptBuff);
//    if (session->lastCtrlMsgTs != 0 &&
//        (timeStamp < session->lastCtrlMsgTs - TS_WND || timeStamp > session->lastCtrlMsgTs + TS_WND)) {
//        DFILE_LOGE(TAG, "timeStamp is out of bound ts is %u, last ts is %u", timeStamp, session->lastCtrlMsgTs);
    if (session->lastCtrlMsgTs != 0 && timeStamp <= session->lastCtrlMsgTs) {
        DFILE_LOGD(TAG, "timeStamp is out of bound ts is %u, last ts is %u", timeStamp, session->lastCtrlMsgTs);
        return false;
    }
//    session->lastCtrlMsgTs = NSTACKX_MAX(timeStamp, session->lastCtrlMsgTs);
    session->lastCtrlMsgTs = timeStamp;
    return true;
}

static int GetDecryptedFrame(DFileSession *session, DFileFrame *dFileFrame)
{
    uint32_t decryptLen = ntohs(dFileFrame->header.length) - GCM_ADDED_LEN;
    uint8_t *decryptBuff = (uint8_t *)malloc(decryptLen);
    if (decryptBuff == NULL) {
        DFILE_LOGE(TAG, "malloc failed");
        return NSTACKX_ENOMEM;
    }
    uint32_t dataLen = ChaCha20Poly1305Decrypt(dFileFrame->payload, ntohs(dFileFrame->header.length), &session->cryptPara,
        decryptBuff, decryptLen);
    if (dataLen == 0) {
        free(decryptBuff);
        DFILE_LOGE(TAG, "ctrlmsg decrypt error, drop it");
        DFILE_LOGE(TAG, "zhanghan here type is %d", dFileFrame->header.type);
        return NSTACKX_EFAILED;
    }
    if (!IsCtrlMsgVaild(session, decryptBuff, dFileFrame)) {
        free(decryptBuff);
        DFILE_LOGE(TAG, "ctrlmsg is invalid, drop it");
        return NSTACKX_EFAILED;
    }
    (void)memcpy_s(dFileFrame, decryptLen - DFILE_CTRL_FRAME_TS_LEN,
        decryptBuff + DFILE_CTRL_FRAME_TS_LEN, decryptLen - DFILE_CTRL_FRAME_TS_LEN);
    free(decryptBuff);
    return NSTACKX_EOK;
}
...
static void ProcessDFileFrameList(DFileSession *session, List *head)
{
    ...
    while (!ListIsEmpty(head) && !session->closeFlag) {
        ...
        if (ntohs(dFileFrame->header.length) > session->maxFrameSize - sizeof(DFileFrameHeader)) {
            DFILE_LOGE(TAG, "header length %u is too big", ntohs(dFileFrame->header.length));
            DestroyQueueNode(queueNode);
            continue;
        }

        if (CapsEncryptCtrlMsg(session) && IsCtrlMsg(dFileFrame) &&
            GetDecryptedFrame(session, dFileFrame) != NSTACKX_EOK) {
            DFILE_LOGE(TAG, "get decrypted frame failed, drop this frame");
            DestroyQueueNode(queueNode);
            continue;
        }
                                    ...
                                    struct sockaddr_in *peerAddr, uint8_t socketIndex)
{
    ...
    QueueNode *queueNode = CreateQueueNode(session, frame, frameLength, peerAddr, socketIndex);
    if (queueNode == NULL) {
        return NSTACKX_ENOMEM;
    }
    ...
    uint16_t connType)
{
    ...
    if (keyLen == 0) {
        DFILE_LOGE(TAG, "not support noencrypt");
        return NSTACKX_EFAILED;
    } else {
        DFILE_LOGI(TAG, "use encrypt");
    }
