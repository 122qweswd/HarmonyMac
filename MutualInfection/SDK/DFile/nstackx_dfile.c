...
static uint32_t g_internalCaps = 0;
    ...
    uint16_t sessionId)
{
    ...
    bool tcp = (g_capabilities & NSTACKX_CAPS_WLAN_CATAGORY) && (g_wlanCatagory == NSTACKX_WLAN_CAT_TCP);
    session->maxFrameSize = tcp ? NSTACKX_TCP_MAX_FRAME_SIZE : NSTACKX_UDP_MAX_FRAME_SIZE;

    session->localUseHeaderEnc = NSTACKX_TRUE;
    session->remoteUseHeaderEnc = NSTACKX_FALSE;
    session->settingUseHeaderEnc = NSTACKX_FALSE;
...
static inline void PostSessionCreate(DFileSession *session)
{
    ...
    session->internalCaps = g_internalCaps | NSTACKX_INTERNAL_CAPS_CLEAR_POLICY|NSTACKX_INTERNAL_CAPS_ENCRYPT_CTRLMSG;
...
static DFileSession *DFileSessionCreate(DFileSessionType type, DFileMsgReceiver msgReceiver)
{
    ...
    session->recvBuffer = calloc(1, session->maxFrameSize);
...
static void DFileSessionClean(DFileSession *session)
{
    ...
    free(session->recvBuffer);
    ClearCryptCtx(session->cryptPara.ctx);
    (void)memset_s(&session->cryptPara, sizeof(CryptPara), 0, sizeof(CryptPara));
    free(session);
    return;
}
...
int32_t NSTACKX_DFileSetCallback(int32_t sessionId, const DFileOptCallback callback)
{
    DFileSessionNode *node = GetDFileSessionNodeById((uint16_t)sessionId);
    if (CheckDFileSessionNodeValid(node) != NSTACKX_EOK) {
        DFILE_LOGE(TAG, "no session node found for id %d", sessionId);
        return NSTACKX_EINVAL;
    }
    
    DFileSession *session = node->session;
    if (session == NULL) {
        DFILE_LOGE(TAG, "no session ctx found for id %d", sessionId);
        return NSTACKX_EINVAL;
    }
    if (session->fileManager == NULL) {
        DFILE_LOGE(TAG, "no file manager found for id %d", sessionId);
        return NSTACKX_EINVAL;
    }
    session->fileManager->fileCallback = callback;
    return NSTACKX_EOK;
}
