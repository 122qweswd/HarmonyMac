...
#define DEFAULT_WRITER_SIZE (5 * 1024 * 1024)
#define DEFAULT_LITEOS_WRITER_SIZE (512 * 1024)
#define WRITER_OUT_TIME_MS (100)

static FileRecvState FileGetRecvStatus(stFileInfo *fileInfo);
typedef struct {
...
int32_t SetFileOffset(stFileInfo *fileInfo, uint64_t fileOffset)
{
...
void CloseFile(stFileInfo *fileInfo)
{
    if (fileInfo == NULL) {
        return;
    }
...
static uint64_t FileGetBytesTransferred(const stFileInfo *fileInfo, uint8_t isSender)
{
...
static void FileInfoWriteInit(stFileInfo *fileInfo, const char *path, uint8_t isTruncate)
{
...
seq_writer *sw_create(int fd, uint32_t cache_size)
{
    if (cache_size == 0) {
//#if defined(NSTACKX_WITH_LITEOS) || defined(NSTACKX_WITH_HMOS_LINUX) || defined(BUILD_FOR_WINDOWS)
//    cache_size = DEFAULT_LITEOS_WRITER_SIZE;
//#else
//    cache_size = DEFAULT_WRITER_SIZE;
//#endif
    cache_size = DEFAULT_WRITER_SIZE;
    }
    seq_writer *sw = malloc(sizeof(*sw));
    if (!sw) {
    ...
    sw->tail    = 0;
    sw->next_off= 0; /* 初始未知，由第一笔设置 */
    ClockGetTime(CLOCK_MONOTONIC, &sw->last);
    return sw;
}

static int sb_flush(seq_writer *sw)
{
    if (sw->tail == 0 || sw->fd < 0) {
        DFILE_LOGE(TAG, "sb_flush failed, sw->tail: %u, sw->fd: %u", sw->tail, sw->fd);
        return 0;
    }
    
    uint32_t ret = 0;
    uint64_t theFileOffset = sw->next_off - sw->tail;

    while (ret < sw->tail) {
        ssize_t written = pwrite(sw->fd, sw->buf + ret, sw->tail - ret, (off_t)theFileOffset);
        if (written < 0 && GetErrno() != EAGAIN) {
            DFILE_LOGE(TAG, "sb_flush failed, sw->fd: %u, written: %u, sw->buf+ret: %p, sw->tail-ret: %u, theFileOffset: %zu",
                       sw->fd, written, sw->buf + ret, sw->tail - ret, theFileOffset);
            sw->tail = 0;
            ClockGetTime(CLOCK_MONOTONIC, &sw->last);
            return -1;
        }
        ret += written;
        theFileOffset += (uint64_t)written;
    }
    sw->tail = 0;
    ClockGetTime(CLOCK_MONOTONIC, &sw->last);
    return 0;
...
int sw_write(int fd, seq_writer *sw, uint64_t off, const void *data, uint32_t len)
{
    /* 第一笔 */
    if (sw->tail == 0 && sw->next_off == 0) {
        sw->next_off = off;
    }

    /* 期望连续 */
    if (off == sw->next_off && fd == sw->fd) {
        /* 缓冲区放得下 */
        if (sw->tail + len <= sw->size) {
            memcpy_s(sw->buf + sw->tail, sw->size - sw->tail, data, len);
            sw->tail     += len;
            sw->next_off += len;
        } else {
            /* 先刷盘，再追加 */
            if (sb_flush(sw) < 0) {
                DFILE_LOGE(TAG, "sb_flush failed");
                return -1;
            }
            memcpy_s(sw->buf, sw->size, data, len);
            sw->tail     = len;
            sw->next_off = off + len;
        }
    } else {
        ...
        sw->fd = fd;
        sw->next_off = off;
        return sw_write(fd, sw, off, data, len); /* 递归一次即可 */
    }

    /* 超时或满则刷 */
    struct timespec now;
    ClockGetTime(CLOCK_MONOTONIC, &now);
    uint64_t measureElapse = GetTimeDiffMs(&now, &sw->last);
    if (sw->tail >= sw->size || measureElapse >= WRITER_OUT_TIME_MS) {
        if (sb_flush(sw) != 0) {
            DFILE_LOGE(TAG, "sb_flush failed");
            return -1;
        }
...
int sw_close(seq_writer *sw, int do_fsync)
{
    if (sw == NULL) {
        return 0;
    }
...
static int32_t WriteToFile(stFileInfo *fileInfo, uint32_t blockSequence, uint16_t length, uint8_t *payLoad,
    FileListTask *fileList, seq_writer *sw)
{
    uint64_t fileOffset;
    uint16_t ret = 0;
    int32_t pRet = 0;
    ...
    if (fileInfo->fd == NSTACKX_INVALID_FD) {
        ...
        if (fileManager->fileCallback) {
            fileInfo->fd = fileManager->fileCallback(session->sessionId, FILE_RECV_START, fileInfo->fileName, NULL, 0);
        }
        if (fileInfo->fd == NSTACKX_INVALID_FD) {
            return NSTACKX_EFAILED;
        }
    ...
    if (CapsNoRW(session)) {
        ...
        FileManager *fileManager = session->fileManager;
        if (fileManager->fileCallback) {
            fileManager->fileCallback(session->sessionId, FILE_RECV_DATA, fileInfo->fileName, NULL, length);
        }
        pRet = sw_write(fileInfo->fd, sw, fileOffset, payLoad, length);
        if (pRet >= 0) {
            ret = (uint16_t)pRet;
        }
#endif
    ...
    uint32_t *blockSequence, uint16_t *payloadLength)
{
    ...
    *blockSequence = ntohl(blockFrame->fileDataFrame->blockSequence);
    ...
    if (transId != fileList->transId || *fileId > fileList->fileNum || *fileId == 0) {
        ...
        stFileInfo *info = &fileList->fileInfo[*fileId - 1];
    ...
    if (*blockSequence >= fileList->fileInfo[*fileId - 1].totalBlockNum ||
        length <= sizeof(FileDataFrame) - sizeof(DFileFrameHeader) || length > fileList->maxFrameLength) {
...
static void UpdateFileListRecvStatus(FileManager *fileManager, FileListTask *fileList, stFileInfo *fileInfo, int32_t ret)
{
    if (ret != NSTACKX_EOK) {
    ...
    if (fileManager->fileCallback) {
        DFileSession *session = fileList->context;
        fileManager->fileCallback(session->sessionId, FILE_RECV_END, fileInfo->fileName, NULL, 0);
    }
}

static int32_t CheckHeaderEnc(BlockFrame* blockFrame, const uint8_t* payload, uint16_t fileId, uint32_t blockSequence)
{
    uint16_t transId = ntohs(blockFrame->fileDataFrame->header.transId);
    uint16_t decryptFileId, decryptTransId;
    uint32_t decryptBlockSequence;
    uint32_t offset = 0;
    int32_t ret = NSTACKX_EOK;

    if (payload == NULL) {
        DFILE_LOGI(TAG, "payload is null. transId/%u fileId/%u blockSequence/%u", transId, fileId, blockSequence);
        return NSTACKX_EOK;
    }

    decryptFileId = ntohs(*(uint16_t*)(payload + offset));
    offset += sizeof(decryptFileId);
    decryptBlockSequence = ntohl(*(uint32_t*)(payload + offset));
    offset += sizeof(decryptBlockSequence);
    decryptTransId = ntohs(*(uint16_t*)(payload + offset));
    offset += sizeof(decryptTransId);

    if (fileId != decryptFileId || blockSequence != decryptBlockSequence || transId != decryptTransId) {
        blockFrame->fileDataFrame->header.transId = htons(decryptTransId);
        blockFrame->fileDataFrame->fileId = htons(decryptFileId);
        blockFrame->fileDataFrame->blockSequence = htonl(decryptBlockSequence);
        DFILE_LOGE(TAG, "check header enc failed. fileId %u/%u blockSequence %u/%u transId %u/%u", fileId, decryptFileId, blockSequence, decryptBlockSequence, transId, decryptTransId);
...
static int32_t WriteFaildOnRetry(DFileSession* session, BlockFrame* blockFrame, FileListTask* fileList, stFileInfo* fileInfo)
{
    session->decContinuousError++;
    fileList->recvFileProcessed++;
    NotifyFileMsg(fileList, fileInfo->fileId, FILE_MANAGER_RECEIVE_FAIL);
    return NSTACKX_EFAILED;
}

static int32_t PayLoadDecrypt(uint8_t **payLoad, uint16_t *payloadLength, FileListTask *fileList,
                              stFileInfo *fileInfo, uint8_t **buffer)
{
#if DFILE_SUPPORT_ENCRYPT
    uint32_t dataLen;
    if (fileList->cryptPara.keylen > 0 && (*payloadLength) > 0) {
        *buffer = (uint8_t *)calloc(*payloadLength, 1);
        ...
        dataLen = ChaCha20Poly1305Decrypt((*payLoad), *payloadLength, &fileList->cryptPara, *buffer, *payloadLength);
...
static int32_t WriteSingleBlockFrame(FileManager *fileManager, FileListTask *fileList, BlockFrame *blockFrame)
{
    ...
    uint16_t headerEncLen = UseHeaderEnc(session)?GetHeaderEncLen():0;
    uint32_t standardBlockSize = GetStandardBlockSize(fileManager);
    ...
    if (PayLoadDecrypt(&payLoad, &payloadLength, fileList, fileInfo, &buffer) != NSTACKX_EOK) {
        DFILE_LOGE(TAG, "decrypt failed");
        return WriteFaildOnRetry(session, blockFrame, fileList, fileInfo);
    }

    if (headerEncLen > 0) {
        if (payloadLength < headerEncLen || CheckHeaderEnc(blockFrame, payLoad, fileId, blockSequence) != NSTACKX_EOK) {
            return WriteFaildOnRetry(session, blockFrame, fileList, fileInfo);
        }
    }

    if (payLoad != NULL) {
        payloadLength -= headerEncLen;
        ret = WriteToFile(fileInfo, blockSequence, payloadLength, payLoad + headerEncLen, fileList, fileManager->writeBuff);
        if (ret == NSTACKX_EOK) {
...
static void GenerateAllEmptyFiles(FileListTask *fileList)
{
    ...
    for (uint32_t i = 0; i < fileList->fileNum; i++) {
        ...
        ret = WriteToFile(fileInfo, 0, 0, NULL, fileList, ((DFileSession *)fileList->context)->fileManager->writeBuff);
        CloseFile(fileInfo);
        fileList->recvFileProcessed++;
...
uint16_t GetStandardBlockSize(const FileManager *fileManager)
{
    ...
    if (UseHeaderEnc(session)) {
        uint16_t headerEncLen = GetHeaderEncLen();
        if (standardBlockSize < headerEncLen) {
            DFILE_LOGE(TAG, "standardBlockSize/%u less than headerEncLen/%u", standardBlockSize, headerEncLen);
            return 0;
        }
        standardBlockSize -= headerEncLen;
    }

    if (standardBlockSize == 0) {
        DFILE_LOGE(TAG, "standardBlockSize is 0");
    }
...
int32_t FileManagerSetMaxFrameLength(FileManager *fileManager, uint16_t maxFrameLength)
{
    ...
    if (maxFrameLength <= offsetof(FileDataFrame, blockPayload) || maxFrameLength > ((DFileSession*)fileManager->context)->maxFrameSize) {
...
static int32_t InitAllCacheList(FileManager *fileManager)
{
    ...
    if (fileManager->isSender) {
        ...
        fileManager->readBuff = sr_create(-1, 0);
        if (fileManager->readBuff == NULL) {
            MutexListDestory(&fileManager->taskList);
            DFILE_LOGE(TAG, "readBuff Init error");
            goto L_ERR_Sender_MANAGER;
        }
    } else {
        fileManager->writeBuff = sw_create(-1, 0);
        if (fileManager->writeBuff == NULL) {
            MutexListDestory(&fileManager->taskList);
            DFILE_LOGE(TAG, "writeBuff Init error");
            return NSTACKX_EFAILED;
        }
    }
    return NSTACKX_EOK;
L_ERR_Sender_MANAGER:
    for (uint32_t i = 0; i < fileManager->sendFrameListNum; i++) {
...
L_ERR_FILE_MANAGER:
    ...
    if (fileManager->isSender) {
        ...
        sr_close(fileManager->readBuff);
    } else {
        sw_close(fileManager->writeBuff, 0);
    }
    (void)memset_s(fileManager, sizeof(FileManager), 0, sizeof(FileManager));
    free(fileManager);
    return NULL;
...
void FileManagerDestroy(FileManager *fileManager)
{
    ...
    if (fileManager->isSender) {
        ...
        sw_close(fileManager->writeBuff, 1);
    }
