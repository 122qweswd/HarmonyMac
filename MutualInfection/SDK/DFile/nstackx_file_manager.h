...
typedef struct {
    ...
    uint16_t fileNum;
    int32_t errCode;
    stFileInfo fileInfo[NSTACKX_DFILE_MAX_FILE_NUM];
    sem_t *semStop;
...
typedef struct {
    ...
    uint8_t *buf;
    uint32_t size;
    uint32_t tail;      /* 已缓存字节数 */
    uint64_t next_off;  /* 期望下一笔 offset */
...
typedef struct {
    ...
    uint8_t *buf;
    uint32_t size;       /* 缓冲区大小 */
    uint64_t window_off; /* 缓冲区在文件中的起始偏移 */
    uint32_t window_len; /* 缓冲区有效长度 */
} seq_reader;
...
int sw_write(int fd, seq_writer *sw, uint64_t offset, const void *data, uint32_t len);
...
void FileSync(const stFileInfo *fileInfo);
...
/* 创建，cache_size 建议 1 MB */
seq_writer *sw_create(int fd, uint32_t cache_size);
 
/* 写入：仅接受连续偏移，否则内部先刷盘再对齐 */
int sw_write(int fd, seq_writer *sw, uint64_t offset, const void *data, uint32_t len);
 
/* 关闭，do_fsync=1 会 fsync */
int sw_close(seq_writer *sw, int do_fsync);
#ifdef __cplusplus
}
#endif
