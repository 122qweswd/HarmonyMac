...
static void GetFileConfigP2p(DFileConfig *dFileConfig, uint16_t mtu, uint32_t dataFrameSize)
{
    dFileConfig->dataFrameSize = NSTACKX_P2P_FRAME_SIZE_TIMES * mtu;
    if (dataFrameSize != 0) {
        dFileConfig->dataFrameSize = dataFrameSize >= UINT16_MAX ? UINT16_MAX : (uint16_t)dataFrameSize;
    }
...
static void GetFileConfigWlan(DFileConfig *dFileConfig, uint16_t mtu, uint32_t dataFrameSize)
{
...
int32_t GetDFileConfig(DFileConfig *dFileConfig, uint16_t mtu, uint16_t connType, uint32_t dataFrameSize)
{
    if ((dFileConfig == NULL) || CheckConnType(connType) != NSTACKX_EOK || (mtu == 0)) {
        return NSTACKX_EINVAL;
    }
    if (connType == CONNECT_TYPE_P2P) {
        GetFileConfigP2p(dFileConfig, mtu, dataFrameSize);
    } else {
        GetFileConfigWlan(dFileConfig, mtu, dataFrameSize);
    }
    return NSTACKX_EOK;
}
