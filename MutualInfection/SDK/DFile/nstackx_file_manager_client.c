...
#define DEFAULT_READER_SIZE (5 * 1024 * 1024)
#define DEFAULT_LITEOS_READER_SIZE (512 * 1024)
...
seq_reader *sr_create(int fd, uint32_t cache_size)
{
    ...
    cache_size = DEFAULT_LITEOS_READER_SIZE;
#else
    cache_size = DEFAULT_READER_SIZE;
#endif
        cache_size = DEFAULT_READER_SIZE;
    }
    seq_reader *sr = malloc(sizeof(*sr));
    if (!sr) {
        return NULL;
    }
    sr->fd         = fd;
    sr->size       = cache_size;
    sr->buf        = malloc(cache_size);
    if (!sr->buf) {
        free(sr);
        return NULL;
    }
...
static int sr_fill_window(seq_reader *sr, uint64_t need_off)
{
    /* 对齐到 512 KB 边界 */
    uint64_t ra_off = 0;
    uint32_t ra_len = 0;
#if defined(NSTACKX_WITH_LITEOS) || defined(NSTACKX_WITH_HMOS_LINUX) || defined(BUILD_FOR_WINDOWS)
    ra_off = need_off & ~(uint64_t)(DEFAULT_LITEOS_READER_SIZE - 1);
    ra_len = DEFAULT_LITEOS_READER_SIZE;
#else
    ra_off = need_off & ~(uint64_t)(DEFAULT_READER_SIZE - 1);
    ra_len = DEFAULT_READER_SIZE;
#endif
    if (ra_off + ra_len < ra_off) {
        ra_len = 0; /* 溢出 */
    }
 
    ssize_t n = pread(sr->fd, sr->buf, ra_len, ra_off);
    if (n < 0) {
        return -1;
    }
    sr->window_off = ra_off;
    sr->window_len = n;
    return 0;
}

int sr_read(int fd, seq_reader *sr, uint64_t off, void *dst, uint32_t len)
{
    /* 边界检查 */
    if (off + len < off) {
        return -EINVAL;
    ...
    /* 完全在窗口内？ */
    if (fd == sr->fd && off >= sr->window_off &&
        off + len <= sr->window_off + sr->window_len) {
        memcpy_s(dst, len, sr->buf + (off - sr->window_off), len);
        return len;
    }
    sr->fd = fd;
    /* 未命中，重新填窗口 */
    if (sr_fill_window(sr, off) < 0) {
        return -1;
    }
 
    /* 再次检查 */
    if (off >= sr->window_off &&
        off + len <= sr->window_off + sr->window_len) {
        memcpy_s(dst, len, sr->buf + (off - sr->window_off), len);
        return len;
    }
    /* 请求长度 > 窗口大小，直接 bypass */
    return pread(sr->fd, dst, len, off) == len ? len : -1;
}
...
static int32_t ReadFromFile(FileManager *fileManager, stFileInfo *fileInfo, uint64_t offset, uint8_t *buffer,
...
//        fileInfo->fd = open(fileInfo->fileName, O_RDONLY);
        if (fileManager->fileCallback != NULL) {
            fileInfo->fd = fileManager->fileCallback(session->sessionId, FILE_SEND_START, fileInfo->fileName, NULL, 0);
        }
#endif
        if (fileInfo->fd == NSTACKX_INVALID_FD) {
...
//        readLength = (uint32_t)sr_read(fileInfo->fd, fileManager->readBuff, (int64_t)offset, buffer, bufferLength);
//        if (fileManager->fileCallback != NULL) {
//            fileManager->fileCallback(session->sessionId, FILE_SEND_DATA, fileInfo->fileName, buffer, bufferLength);
//            readLength = bufferLength;
//        }
#endif
    }
...
static int32_t WriteHeaderEnc(uint8_t* buffer, uint16_t bufferLen, uint16_t fileId, uint32_t blockSequence, uint16_t transId)
{
    uint32_t offset = 0, valSize = 0;

    fileId = htons(fileId);
    blockSequence = htonl(blockSequence);
    transId = htons(transId);

    if (buffer == NULL) {
        DFILE_LOGE(TAG, "buffer is null. bufferLen/%u, transId/%u, fileId/%u, blockSequence/%u", bufferLen, transId, fileId, blockSequence);
        return NSTACKX_EFAILED;
    }

    valSize = sizeof(fileId);
    if (memcpy_s(buffer + offset, bufferLen - offset, &fileId, valSize) != EOK) {
        DFILE_LOGE(TAG, "write fileId failed. bufferLen/%u, transId/%u, fileId/%u, blockSequence/%u", bufferLen, transId, fileId, blockSequence);
        return NSTACKX_EFAILED;
    }

    offset += valSize;

    valSize = sizeof(blockSequence);
    if (memcpy_s(buffer + offset, bufferLen - offset, &blockSequence, valSize) != EOK) {
        DFILE_LOGE(TAG, "write blockSequence failed. bufferLen/%u, transId/%u, fileId/%u, blockSequence/%u", bufferLen, transId, fileId, blockSequence);
        return NSTACKX_EFAILED;
    }

    offset += valSize;

    valSize = sizeof(transId);
    if (memcpy_s(buffer + offset, bufferLen - offset, &transId, valSize) != EOK) {
        DFILE_LOGE(TAG, "write transId failed. bufferLen/%u, transId/%u, fileId/%u, blockSequence/%u", bufferLen, transId, fileId, blockSequence);
        return NSTACKX_EFAILED;
    }
...
static uint8_t* PrepareFrameData(FileManager* fileManager, stFileInfo* fileInfo, uint16_t targetLength,
                                uint16_t headerEncLen, uint64_t fileOffset, uint32_t targetSequence, uint16_t transId)
{
    uint8_t* buffer = NULL;
    ...
    buffer = (uint8_t*)calloc(headerEncLen+targetLength, 1);
    if (buffer == NULL) {
    ...
    if (headerEncLen > 0) {
        if (WriteHeaderEnc(buffer, headerEncLen, fileInfo->fileId, targetSequence, transId) != NSTACKX_EOK) {
            free(buffer);
            return NULL;
        }
    }
    if (ReadFromFile(fileManager, fileInfo, fileOffset, buffer + headerEncLen, targetLength) != NSTACKX_EOK) {
        free(buffer);
        return NULL;
    }
...
static FileDataFrame *GetEncryptedDataFrame(FileManager *fileManager, CryptPara *cryptPara, stFileInfo *fileInfo,
                                            uint32_t targetSequence, uint16_t transId)
{
    uint8_t *buffer = NULL;
    ...
    buffer = PrepareFrameData(fileManager, fileInfo, targetLenth, headerEncLen, fileOffset, targetSequence, transId);
    if (buffer == NULL) {
        return NULL;
    }
    fileManager->iorBytes += (uint64_t)targetLenth;
    targetLenth += headerEncLen;
    ...
    if (ChaCha20Poly1305Encrypt(buffer, targetLenth, cryptPara, (uint8_t *)fileDataFrame + frameOffset, payLoadLen) == 0) {
        fileInfo->errCode = FILE_MANAGER_FILE_EOTHER;
        free(fileDataFrame);
        fileDataFrame = NULL;
...
static FileDataFrame *GetNoEncryptedDataFrame(FileManager *fileManager, stFileInfo *fileInfo, uint32_t targetSequence)
{
    uint16_t frameOffset, targetLenth;
    FileDataFrame *fileDataFrame = NULL;
    uint64_t fileOffset;
...
int32_t GetEncryptedDataTarFrame(CryptPara *cryptPara, uint16_t fileId, FileListTask *fileList, uint16_t targetLenth)
{
    ...
    fileDataFrame->blockSequence = htonl(targetSequence);
    if (ChaCha20Poly1305Encrypt(buffer->blockPayload, targetLenth, cryptPara, fileDataFrame->blockPayload, payLoadLen) == 0) {
        DFILE_LOGE(TAG, "ChaCha20Poly1305Encrypt failed, %d-%d", targetLenth, payLoadLen);
...
FileDataFrame *CreateRetranBlockFrame(FileManager *fileManager, FileListTask *fileList)
{
    ...
    if (fileList->cryptPara.keylen > 0) {
        fileDataFrame = GetEncryptedDataFrame(fileManager, &fileList->cryptPara, fileInfo, blockSequence, fileList->transId);
    } else {
...
static stFileInfo *GetTarFileInfo(FileListTask *fileList)
{
    (void)fileList;
    return NULL;
}

static stFileInfo *GetFileInfo(FileListTask *fileList)
{
    uint16_t fileId;
...
static void UpdateFileLisSendStatus(FileListTask *fileList, stFileInfo *fileInfo, uint8_t *isEnd)
{
...
//            DFileSession *session = fileList->sess
//            if (
        }
...
void UpdateTarFileListSendStatus(FileListTask *fileList)
{
    ...
    if (fileList->tarFileInfo.maxSequenceSend == (int32_t)(fileList->tarFileInfo.totalBlockNum - 1)) {
        ...
        if (MAX_SEND_FILE_OPENED_PER_LIST <= 1) { // Macro maybe changed
            stFileInfo *fileInfo = GetFileInfo(fileList);
            if (fileInfo != NULL) {
                CloseFile(fileInfo);
            }
        }
...
static FileDataFrame *CreateSendBlockFrame(FileManager *fileManager, FileListTask *fileList)
{
    ...
    if (fileList->cryptPara.keylen > 0) {
        fileDataFrame = GetEncryptedDataFrame(fileManager, &fileList->cryptPara, fileInfo,
            (uint32_t)(fileInfo->maxSequenceSend + 1), fileList->transId);
    } else {
                               ...
                               FileListTask *fmFileList, uint16_t standardBlockSize)
{
    ...
    for (i = 0; i < fmFileList->fileNum; i++) {
        ...
        fileInfo->fileName = strdup(fileListInfo->fileList[i]);//realpath(fileListInfo->fileList[i], NULL);
