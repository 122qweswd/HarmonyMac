...
typedef enum {
    DFILE_ON_FILE_RECV_START,
    DFILE_ON_FILE_RECV_DATA,
    DFILE_ON_FILE_RECV_END,
} DFileMsgTypeExt;
...
// typedef int (*DFileOptCallback)(int32_t sessionId, FileCallbackType type, const char *fileName, void *data, uint64_t len);
typedef int (*DFileOptCallback)(int32_t sessionId, FileCallbackType type, const char *fileName, void *data, uint64_t len);
/*
...
NSTACKX_EXPORT int32_t NSTACKX_DFileSetCallback(int32_t sessionId, const DFileOptCallback callback);
#ifdef __cplusplus
}
#endif
