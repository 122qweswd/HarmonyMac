...
#define NSTACKX_DEFAULT_FRAME_SIZE_V6       1452
...
#define DFILE_CTRL_FRAME_TS_LEN sizeof(uint64_t)
#define DFILE_CTRL_FRAME_EXT_LEN (DFILE_FRAME_HEADER_LEN + DFILE_CTRL_FRAME_TS_LEN)
...
typedef enum {
    ...
    NSTACKX_DFILE_FILEID_SUCC_FRAME,
    NSTACKX_DFILE_FILE_HEADER_FRAME_V2,
    NSTACKX_DFILE_FILE_LARGE_DATA_FRAME,
    NSTACKX_DFILE_SESSION_OPT_FRAEM,
...
#define NSTACKX_TCP_MAX_FRAME_SIZE  ((1 << (8 * sizeof(((DFileFrameHeader){0}).length))) - 1)
#define NSTACKX_UDP_MAX_FRAME_SIZE  14720
#define NSTACKX_MAX_FRAME_SIZE          NSTACKX_TCP_MAX_FRAME_SIZE
typedef struct {
    uint16_t fileId;
    uint64_t fileSize;
...
static inline uint8_t GetHeaderEncLen()
{
    FileDataFrame frame;
    return sizeof(frame.fileId) + sizeof(frame.blockSequence) + sizeof(frame.header.transId);
}
...
uint32_t DFileGetFrameLen(const struct sockaddr_in *addr);
