...
uint32_t ChaCha20Poly1305Encrypt(const uint8_t *inBuf, uint32_t inLen, CryptPara *cryptPara, uint8_t *outBuf,
                                 uint32_t outLen)
{
    // 检查参数
    if (inBuf == NULL || cryptPara == NULL || outBuf == NULL) {
        LOGE(TAG, "Invalid parameters");
        return 0;
    }

    // 检查输出缓冲区大小
    if (outLen < inLen + crypto_aead_chacha20poly1305_ietf_ABYTES + crypto_aead_chacha20poly1305_ietf_NPUBBYTES) {
        LOGE(TAG, "Output buffer too small");
        return 0;
    }

    // 检查密钥长度
    if (cryptPara->keylen != crypto_aead_chacha20poly1305_ietf_KEYBYTES) {
        LOGE(TAG, "Invalid key length");
        return 0;
    }

    // 初始化libsodium
    if (sodium_init() < 0) {
        LOGE(TAG, "Failed to initialize libsodium");
        return 0;
    }

    // 生成随机nonce
    cryptPara->ivLen = crypto_aead_chacha20poly1305_ietf_NPUBBYTES;
    randombytes_buf(cryptPara->iv, cryptPara->ivLen);

    // 执行加密
    unsigned long long ciphertext_len = 0;
    int result = crypto_aead_chacha20poly1305_ietf_encrypt(
        outBuf, &ciphertext_len,
        inBuf, inLen,
        cryptPara->aad, cryptPara->aadLen,
        NULL, // nsec - not used
        cryptPara->iv,
        cryptPara->key
    );
    ...
    if (result != 0) {
        LOGE(TAG, "ChaCha20-Poly1305 encryption failed with code %d", result);
        return 0;
    }

    // 将nonce附加到密文末尾
    if (memcpy_s(outBuf + ciphertext_len, outLen - ciphertext_len, cryptPara->iv, cryptPara->ivLen) != EOK) {
        LOGE(TAG, "Failed to append nonce");
        return 0;
    }

    return (uint32_t)(ciphertext_len + cryptPara->ivLen);
}
...
// ChaCha20-Poly1305 解密实现
uint32_t ChaCha20Poly1305Decrypt(uint8_t *inBuf, uint32_t inLen, CryptPara *cryptPara, uint8_t *outBuf,
                                 uint32_t outLen)
{
    // 检查参数
    if (inBuf == NULL || cryptPara == NULL || outBuf == NULL) {
    ...
    // 检查输入长度
    if (inLen <= crypto_aead_chacha20poly1305_ietf_ABYTES + crypto_aead_chacha20poly1305_ietf_NPUBBYTES) {
        LOGE(TAG, "Input data too short");
        return 0;
    }
    ...
    // 从输入数据末尾提取nonce
    const unsigned char *nonce = inBuf + (inLen - crypto_aead_chacha20poly1305_ietf_NPUBBYTES);
    uint32_t dataLen = inLen - crypto_aead_chacha20poly1305_ietf_NPUBBYTES;

    // 执行解密
    unsigned long long plaintext_len = 0;
    int result = crypto_aead_chacha20poly1305_ietf_decrypt(
        outBuf, &plaintext_len,
        NULL, // nsec - not used
        inBuf, dataLen,
        cryptPara->aad, cryptPara->aadLen,
        nonce,
    ...
    if (result != 0) {
        LOGE(TAG, "ChaCha20-Poly1305 decryption failed with code %d", result);
        return 0;
    }

    return (uint32_t)plaintext_len;
}
