...
{
    uint32_t fileNum;
    uint16_t encodeLen = CapsEncryptCtrlMsg(dFileTrans->session) ?
        (dFileTrans->mtu - DFILE_CTRL_FRAME_EXT_LEN - GCM_ADDED_LEN) : dFileTrans->mtu;
    uint8_t *encodeBuff = CapsEncryptCtrlMsg(dFileTrans->session) ?
        (dFileTrans->encodeBuffer + DFILE_CTRL_FRAME_TS_LEN) : dFileTrans->sendBuffer;
    ...
    do {
        ...
        (void)memset_s(dFileTrans->encodeBuffer, sizeof(dFileTrans->encodeBuffer), 0, sizeof(dFileTrans->encodeBuffer));

        int32_t lastEncodeHeaderFileId = dFileTrans->lastSentHeaderFileId;

        EncodeFileHeaderFrame(dFileTrans->fileList, &lastEncodeHeaderFileId, encodeBuff,
            encodeLen, &dFileTrans->sendBufferLength);
        if (CapsEncryptCtrlMsg(dFileTrans->session) && GetEncryptedFrame(dFileTrans, encodeBuff) != NSTACKX_EOK) {
...
static void SendFileTransferDoneAckFrame(DFileTrans *dFileTrans, DFileSendState *nextState)
{
    ...
    (void)memset_s(dFileTrans->encodeBuffer, sizeof(dFileTrans->encodeBuffer), 0, sizeof(dFileTrans->encodeBuffer));
    EncodeFileTransferDoneAckFrame(encodeBuff, encodeLen, dFileTrans->transId, &dFileTrans->sendBufferLength);
...
static void SendFileHeaderConfirm(DFileTrans *dFileTrans, DFileReceiveState *nextState)
{
    ...
    do {
        ...
        EncodeFileHeaderConfirmFrame(dFileTrans->fileList, &lastEncAckedHeaderFileId,
                                     encodeBuff, encodeLen, &dFileTrans->sendBufferLength);
        if (CapsEncryptCtrlMsg(dFileTrans->session) && GetEncryptedFrame(dFileTrans, encodeBuff) != NSTACKX_EOK) {
            DFILE_LOGE(TAG, "get encrypt frame failed");
...
static void SendFileTransferDoneFrame(DFileTrans *dFileTrans, DFileReceiveState *nextState)
{
     ...
     */
    EncodeFileTransferDoneFrame(encodeBuff, encodeLen, fileIdList, fileIdNum,
...
{
    if (mtu <= offsetof(FileDataFrame, blockPayload) || mtu > dFileTrans->session->maxFrameSize) {
...
static int GetClockTimeStamp(clockid_t clkId, uint64_t *stamp)
{
    struct timespec now;
    if (clock_gettime(clkId, &now) != 0) {
        DFILE_LOGE(TAG, "clock_gettime error: %d", errno);
        return NSTACKX_EFAILED;
    }
    *stamp = (uint64_t)now.tv_sec * (uint64_t)S_TO_US + (uint64_t)now.tv_nsec / (uint64_t)US_TO_NS;
    return NSTACKX_EOK;
}

static void EncodeDfileFramePlaintExtHdr(uint8_t *dstBuf, uint8_t *srcBuf)
{
    (void)memcpy_s(dstBuf, DFILE_FRAME_HEADER_LEN, srcBuf, DFILE_FRAME_HEADER_LEN);
    ((DFileFrameHeader *)dstBuf)->length = htons(ntohs(((DFileFrameHeader *)dstBuf)->length) +
        DFILE_CTRL_FRAME_EXT_LEN + GCM_ADDED_LEN);
}

int GetEncryptedFrame(DFileTrans *dFileTrans, uint8_t *encodeBuff)
{
    uint64_t timeStamp;
    if (GetClockTimeStamp(CLOCK_MONOTONIC, &timeStamp) != NSTACKX_EOK) {
        return NSTACKX_EFAILED;
    }
    *(uint64_t *)dFileTrans->encodeBuffer = htobe64(timeStamp);
    dFileTrans->sendBufferLength += DFILE_CTRL_FRAME_TS_LEN;
    SetDfileFrameTransID((DFileFrame *)encodeBuff, dFileTrans->transId);
    EncodeDfileFramePlaintExtHdr(dFileTrans->sendBuffer, encodeBuff);

    if (ChaCha20Poly1305Encrypt(dFileTrans->encodeBuffer, dFileTrans->sendBufferLength, &dFileTrans->session->cryptPara,
        dFileTrans->sendBuffer + DFILE_FRAME_HEADER_LEN, dFileTrans->sendBufferLength + GCM_ADDED_LEN) == 0) {
        DFILE_LOGE(TAG, "data encrypt failed");
        return NSTACKX_EFAILED;
    }
    dFileTrans->sendBufferLength += GCM_ADDED_LEN;
    dFileTrans->sendBufferLength += DFILE_FRAME_HEADER_LEN;
    return NSTACKX_EOK;
}

int GetEncryptedFrameWithoutTrans(uint8_t *plaintBuff, uint8_t *outBuff, size_t *len, DFileSession *session)
{
    uint64_t timeStamp;
    if (GetClockTimeStamp(CLOCK_MONOTONIC, &timeStamp) != NSTACKX_EOK) {
        return NSTACKX_EFAILED;
    }
    *(uint64_t *)plaintBuff = htobe64(timeStamp);
    *len += DFILE_CTRL_FRAME_TS_LEN;
    EncodeDfileFramePlaintExtHdr(outBuff, plaintBuff + DFILE_CTRL_FRAME_TS_LEN);
    if (ChaCha20Poly1305Encrypt(plaintBuff, *len, &session->cryptPara,
        outBuff + DFILE_FRAME_HEADER_LEN, *len + GCM_ADDED_LEN) == 0) {
        DFILE_LOGE(TAG, "data encrypt failed");
        return NSTACKX_EFAILED;
    }
    *len += GCM_ADDED_LEN;
    *len += DFILE_FRAME_HEADER_LEN;
    return NSTACKX_EOK;
}
