...
import { __ as writeConfigFile, p_ as readConfigFileSnapshot } from "./reply-VlVWWWSj.js";
...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./reply-VlVWWWSj.js").then((n) => n.Et);
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BP3s2GtQ.js").then((n) => n.t);
		const { logConfigUpdated } = await import("./logging-CEGtsPZe.js").then((n) => n.r);
	...
	const { ensureAuthProfileStore } = await import("./auth-profiles.runtime-BQ0fN_Pb.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-CURvLzIm.js").then((n) => n.t);
	const { promptCustomApiConfig } = await import("./onboard-custom-CMuYBQMy.js").then((n) => n.r);
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-Bc2-u9UN.js").then((n) => n.t);
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-BeIkiqcE.js").then((n) => n.i);
	...
	const { configureGatewayForOnboarding } = await import("./onboarding.gateway-config-CEKhzSS0.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-QyjeD2Ab.js").then((n) => n.n);
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CvF_gRpR.js").then((n) => n.t);
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-Bq0ioKUE.js");
