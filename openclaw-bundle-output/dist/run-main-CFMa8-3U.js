...
import { A_ as loadDotEnv, yg as installUnhandledRejectionHandler } from "./auth-profiles-CUUGuGNp.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-CbscuDxf.js";
import { t as assertSupportedRuntime } from "./runtime-guard-BGuMyQeT.js";
import { t as emitCliBanner } from "./banner-BOMlDbsx.js";
import "./doctor-config-flow-DdQHXMzN.js";
import { n as ensureConfigReady } from "./config-guard-Byx1gHnJ.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-B963887J.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-BTn1xYPY.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-BDl1_9Gp.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-GA8oE04i.js");
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./auth-profiles-CUUGuGNp.js").then((n) => n.V);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-DHzkl8M2.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-DHzkl8M2.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-amdRM-Yn.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-amdRM-Yn.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-BaYMJaPE.js").then((n) => n.n);
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		const { buildProgram } = await import("./program-gqsYZIuG.js");
		...
		if (primary) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BRZIp9i_.js").then((n) => n.t);
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-CfbZMqV7.js").then((n) => n.a);
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-CfbZMqV7.js").then((n) => n.a);
