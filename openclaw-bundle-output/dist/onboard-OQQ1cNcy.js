...
import { At as randomToken, Ct as handleReset, Ft as waitForGatewayReachable, Tt as normalizeGatewayTokenInput, __ as writeConfigFile, _t as applyWizardMetadata, bt as ensureWorkspaceAndSessions, gt as DEFAULT_WORKSPACE, jt as resolveControlUiLinks, p_ as readConfigFileSnapshot } from "./reply-VlVWWWSj.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-BUa2ldQV.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-BCehfZ4_.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-TK0cxPPO.js").then((n) => n.i);
