...
public protocol PhotoPickerDataDelegate: AnyObject {
    ...
    /// 将要选择Asset
    func pickerData(_ pickerData: PhotoPickerData, shouldSelectedAsset photoAsset: PhotoAsset, at index: Int) -> Bool
    ...
    /// 即将选择 全选 时调用
    func pickerDataWillSelectAllAsset(_ pickerData: PhotoPickerData)
    
    /// 选择了 全选 之后调用
    func pickerDataDidSelectAllAsset(_ pickerData: PhotoPickerData)
...
open class PhotoPickerData {
    ...
    /// - Returns: 添加结果
    @discardableResult
    open func append(_ photoAsset: PhotoAsset, isFilterEditor: Bool = false) -> Bool {
    ...
    /// - Parameter photoAsset: 对应的PhotoAsset对象
    @discardableResult
    open func append(_ photoAssets: [PhotoAsset], isFilterEditor: Bool = false) -> [PhotoAsset] {
        
        var failAssets: [PhotoAsset] = []
        
        delegate?.pickerDataWillSelectAllAsset(self)
        for photoAsset in photoAssets.reversed() {
            photoAsset.isSelected = true
            selectedAssets.append(photoAsset)
        }
        delegate?.pickerDataDidSelectAllAsset(self)
