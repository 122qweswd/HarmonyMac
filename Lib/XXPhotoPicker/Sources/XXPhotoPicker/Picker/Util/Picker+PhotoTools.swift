...
extension PhotoTools {
    ...
    // swiftlint:enable superfluous_disable_command
    // swiftlint:disable function_body_length
    public static func getCustomPickerConfig() -> PickerConfiguration {
        // swiftlint:enable function_body_length
        var config = PickerConfiguration() 
        config.maximumSelectedVideoCount = 0
        ...
        config.allowSelectedTogether = true
        config.pickerPresentStyle = .none
        

        
        let wxColor = "#007AFF".color
        config.selectOptions = [.gifPhoto, .livePhoto, .video]
        config.albumShowMode = .popup
        config.appearanceStyle = .normal
        config.navigationViewBackgroundColor = "#2E2F30".color
        ...
        config.albumList.cellHeight = 60
        config.albumList.cellBackgroundColor = "#2E2F30".color
        config.albumList.cellSelectedColor = .clear
        config.albumList.albumNameColor = .white
        config.albumList.photoCountColor = .white
        config.albumList.separatorLineColor = "#434344".color.withAlphaComponent(0.6)
        config.albumList.tickColor = wxColor
        ...
        config.albumController.backgroundColor = "#2E2F30".color
        config.albumController.cellBackgroundColor = "#2E2F30".color
        config.albumController.cellSelectedColor = .clear
        config.albumController.albumNameColor = .white
        config.albumController.photoCountColor = .white
        config.albumController.headerTitleColor = .white
        config.albumController.headerButtonTitleColor = wxColor
        ...
        config.photoList.backgroundColor = "#2E2F30".color
        config.photoList.leftNavigationItems = [PhotoImageCancelItemView.self]
        config.photoList.rightNavigationItems = [PhotoPickerSelectItemView.self]
        ...
        config.photoList.cell.selectBox.titleColor = .white
        config.photoList.cell.selectBox.style = .tick
        config.photoList.cell.selectBox.size = CGSize(width: 24, height: 24)
        ...
        config.photoList.bottomView.finishButtonTitleColor = .white
        config.photoList.bottomView.finishButtonBackgroundColor = "#007AFF".color
        config.photoList.bottomView.finishButtonDisableBackgroundColor = "#007AFF".color
        ...
        config.previewView.selectBox.borderWidth = 1
        config.previewView.selectBox.selectBorderWidth = 1
        
        config.previewView.selectBox.backgroundColor = .white.withAlphaComponent(0.05)
        config.previewView.selectBox.selectedBackgroundColor = "#007AFF".color
        
        config.previewView.selectBox.borderColor = .white.withAlphaComponent(0.22)
        config.previewView.selectBox.selectBorderColor = .white
        ...
        config.previewView.bottomView.finishButtonBackgroundColor = "#007AFF".color
        config.previewView.bottomView.finishButtonDarkBackgroundColor = "#007AFF".color
        ...
        config.navigationViewBackgroundColor = "#191919".color.withAlphaComponent(0.85)
        config.navigationTitleColor = .white
        config.navigationTintColor = .white
        config.statusBarStyle = .lightContent
        config.navigationBarStyle = .black
        ...
        /// LivePhoto静音
        config.isLivePhotoMuted = true
        
        /// 最多可以选择的资源数，如果为0则不限制
        config.maximumSelectedCount = 0
        
        /// 资源类型
        config.selectOptions = [.photo, .video, .livePhoto]
        ...
        /// pan选择
        config.photoList.allowSwipeToSelect = true
        
        /// 通过照片列表配置类隐藏底部原图按钮
        config.photoList.bottomView.isHiddenOriginalButton = true
        
        /// 每行显示数量（iPad可能需要调整一下，不然也太太太丑了）
        config.photoList.rowNumber = UIDevice.customIsPad ? 6 : 3
