...
open class PhotoThumbnailView: UIView {
    ...
    open var photoAsset: PhotoAsset? {
        didSet {
            imageView.image = photoAsset?.thumbnailImage
        }
    }
...
//        )
    ...
    /// 获取图片，重写此方法可以修改图片
    open func requestThumbnailImage(targetWidth: CGFloat, completion: ((UIImage?, PhotoAsset) -> Void)? = nil) {
        ...
        if photoAsset.isNetworkAsset || photoAsset.mediaSubType == .localVideo || isNetworkLivePhotoImage {
            ...
            if let thumbnailImage = photoAsset.thumbnailImage {
                completion?(thumbnailImage, photoAsset)
            } else {
                requestID = photoAsset.requestThumbnailImage(targetWidth: thumbnailLoadMode == .simplify ? 10 : targetWidth) { [weak self] (image, photoAsset, info) in
                    guard let self = self else { return }
                    ...
                    photoAsset.thumbnailImage = image
                    completion?(image, photoAsset)
                }
            }
        }
