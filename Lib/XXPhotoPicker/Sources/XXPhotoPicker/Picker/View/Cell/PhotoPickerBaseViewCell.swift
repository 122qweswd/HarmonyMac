...
open class PhotoPickerBaseViewCell: UICollectionViewCell {
    ...
    open var photoAsset: PhotoAsset! {
        didSet {
            ...
            if isRequestDirectly && photoAsset.thumbnailImage == nil {
                request()
            }
        }
    }
    ...
    open func requestThumbnailImage() {
        requestThumbnailImage(targetWidth: config.targetWidth)
    }
    ...
    //open var inICloud: Bool { photoAsset.inICloud }
