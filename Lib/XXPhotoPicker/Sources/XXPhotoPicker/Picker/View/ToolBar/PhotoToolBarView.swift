...
extension PhotoToolBarView {
    ...
    private func updateFinishButtonTitle(_ photoAssets: [PhotoAsset]) {
        ...
        if count > 0 {
            ...
            if type == .picker {
                ...
                previewBtn.setTitle(.textPhotoList.bottomView.previewTitle.text + " (\(count))", for: .normal)
            }
    ...
    private func updatePreviewButtonFrame() {
        if let previewBtn = previewBtn {
            let previewWidth: CGFloat = previewBtn.currentTitle!.localized.width(ofFont: previewBtn.titleLabel!.font, maxHeight: 50)
            previewBtn.size = .init(width: previewWidth, height: 50)
        }
    }
