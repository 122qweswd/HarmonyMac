...
//        config.editor.buttonType = .top
    ...
    func loadFonts() {
        ...
        masks.append(.text("‬‬‬XXPhotoPicker", UIFont.boldSystemFont(ofSize: 50)))
...
//        )
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        var config = XXPhotoPicker.PhotoBrowser.Configuration()
        ...
        XXPhotoPicker.PhotoBrowser.show(
            // 预览的资源数组
            selectedAssets,
            // 当前预览的位置
    ...
    func previewLongPressClick(photoAsset: PhotoAsset, photoBrowser: XXPhotoPicker.PhotoBrowser) {
...
extension PickerResultViewController: PhotoPickerControllerDelegate {
    ...
    func createEditorDocumentPath() {
        let path = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).last! + "/XXPhotoPicker_editor"
    ...
    func pickerController(
        ...
        if isPublish {
            ...
            var fileName = "XXPhotoPicker_editor/"
    ...
    func pickerController(
        ...
        if isPublish {
            var config = editorConfig
            createEditorDocumentPath()
            let fileName = "XXPhotoPicker_editor/" + String.fileName(suffix: "mp4")
            config.urlConfig = .init(fileName: fileName, type: .document)
            return config
        }
        return editorConfig
