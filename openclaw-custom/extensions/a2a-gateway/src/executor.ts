...
/**
 * MIME type to file extension mapping for saving received files.
 * Keys are lowercase MIME types without parameters.
 */
...
/**
 * Strip MIME parameters (e.g. ";charset=utf-8") and lowercase.
 */
...
/**
 * Determine file extension from MIME type, falling back to the original
 * filename extension or ".bin".
 */
function extensionForMime(mimeType: string, originalName: string): string {
  ...
  // Fallback: use extension from original filename if it looks reasonable
...
/**
 * Global temp directory for saved files. Set during executor construction.
 */
let fileTempDir: string = path.join(os.tmpdir(), "a2a-files");

/**
 * Decode a base64-encoded inline file and save it to the temp directory.
...
function saveBase64File(
  ...
  try {
    ...
    const filePath = path.join(fileTempDir, filename);
 ...
 * so the agent can reference or fetch them; inline base64 files are decoded
 * and saved to disk, with the local path included in the text so the agent
...
function formatFilePartAsText(obj: Record<string, unknown>): string {
  ...
  // Base64-encoded inline file: decode and save to temp directory
  const bytes = asString(file.bytes);
  if (bytes) {
    const savedPath = saveBase64File(bytes, name, mimeType);
    if (savedPath) {
      const sizeKB = Math.ceil(decodedBase64Size(bytes) / 1024);
      return `[Attached: ${name} (${mimeType}), ${sizeKB}KB, saved to ${savedPath}]`;
    }
    // Fallback: file save failed, report size only
    const sizeKB = Math.ceil(decodedBase64Size(bytes) / 1024);
    return `[Attached: ${name} (${mimeType}), inline ${sizeKB}KB (save failed)]`;
  }
